...
class ForecastDetailMonth(BaseModel):
    ...
    forecast_detail_id: MappedColumn[str] = mapped_column(
        String, ForeignKey("va_forecast_details.id"), nullable=False
    )
    ...
    detail: MappedColumn["ForecastDetail"] = relationship(
        "ForecastDetail", back_populates="months"
    )
