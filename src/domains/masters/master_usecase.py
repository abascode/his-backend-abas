...
class MasterUseCase(IMasterUseCase):
    ...
    def get_order_configuration(
        self, request: Request, order_configuration: GetOrderConfigurationRequest
    ) -> List[OrderConfigurationsResponse]:
        orders = self.master_repo.get_order_configuration(
            request, order_configuration.month, order_configuration.year
        )
    ...
    def get_stock_pilots(
        self, request: Request, stock_pilots: GetStockPilotRequest
    ) -> List[StockPilotsResponse]:
        stock_pilots = self.master_repo.get_stock_pilots(
            request, stock_pilots.month, stock_pilots.year
        )
    ...
    def get_segment_options(
        ...
        segments = self.master_repo.get_segment_options(request, search)
        return [
            TextValueResponse(
                text=i.id,
                value=i.id,
    ...
    def get_category_options(
        ...
        categories = self.master_repo.get_category_options(request, search)
        return [
            TextValueResponse(
                text=i.id,
                value=i.id,
